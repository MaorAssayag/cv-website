...
const DisplacementSphere = dynamic(
  () => import('@/components/displacement-sphere/DisplacementSphere'),
  { ssr: false }
);
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* 3D Flower / Displacement Sphere Animation */}
      <DisplacementSphere
        className={`transition-opacity duration-700 ${isDSPMode ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
      />
          ...
          <div className="space-y-8">
            <div className="glass p-8 rounded-3xl space-y-4 transition-all duration-500 animate-in fade-in slide-in-from-bottom-4" style={{ animationDuration: '800ms', animationFillMode: 'both' }}>
              ...
              <ul className="space-y-3 text-gray-700 leading-relaxed list-disc list-inside marker:text-gray-300">
                Leading R&D for next-generation multi-beam electron microscopy, combining physics-informed AI, applied science (image processing & numerical algorithms) and software engineering to optimize complex imaging hardware calibrations
              </ul>
            </div>
